...
describe("GatewayStore", () => {
  ...
  it("derives a metrics snapshot from persisted job state", async () => {
    ...
    const grokFailed = await queuedJobWithKey(store, "metrics-grok-limited");
    await store.claimNextJob();
    await store.failJob(grokFailed.job.id, "GROK_RATE_LIMITED", "slow down", true);
    await database.db.updateTable("jobs").set({ completedAt: now }).where("id", "=", grokFailed.job.id).execute();

    const cursorFailed = await queuedJobWithKey(store, "metrics-cursor-limited");
    await store.claimNextJob();
    await store.failJob(cursorFailed.job.id, "CURSOR_RATE_LIMITED", "slow down", true);
    await database.db.updateTable("jobs").set({ completedAt: now }).where("id", "=", cursorFailed.job.id).execute();
    ...
    expect(metrics.jobsByStatus).toEqual({ queued: 1, running: 0, completed: 1, failed: 4, cancelled: 1 });
    expect(metrics.jobsByKind).toEqual({ "coding.turn": 7, "inference.turn": 0 });
    ...
    expect(metrics.window.failuresByErrorCode).toEqual({
      ...
      CLAUDE_RATE_LIMITED: 1,
      GROK_RATE_LIMITED: 1,
      CURSOR_RATE_LIMITED: 1
    });
    expect(metrics.window.rateLimitedByBackend).toEqual({ codex: 1, claude: 1, grok: 1, cursor: 1 });
  ...
  it("excludes rows outside the metrics window", async () => {
    ...
    expect(metrics.window.failuresByErrorCode).toEqual({});
    expect(metrics.window.rateLimitedByBackend).toEqual({ codex: 0, claude: 0, grok: 0, cursor: 0 });
  });
