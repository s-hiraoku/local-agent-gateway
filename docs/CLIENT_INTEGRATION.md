...
V2 is a breaking replacement for the old `/v1/tasks` API. Clients use Gateway coding runs, inference runs, conversations, and jobs; they never send repository paths, Codex IDs, backend credentials, model-provider requests, or raw JSON-RPC payloads. Codex App Server is an internal backend, not a client protocol; see [Codex App Server](CODEX_APP_SERVER.md).
