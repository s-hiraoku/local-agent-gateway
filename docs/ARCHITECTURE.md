...
- `adapters`: backend protocol translation behind one provider-neutral runner contract (`adapters/runner.ts`), currently Codex App Server, Claude Code headless, Grok Build headless, and Cursor SDK;
...
SQLite is the source of truth for conversations, jobs, attempts, idempotency records, event order, and the latest successful retention-sweep result. Prompt, result, and event bodies are AES-256-GCM encrypted. Operational metadata such as job state, timestamps, repository ID, attempt number, and event type remains queryable.
...
Because SQLite is the source of truth, operational metrics (`GET /v2/metrics`) are computed from durable rows rather than kept as in-memory counters. This is deliberate: an always-on service redeploys often (there is a `GATEWAY_RESTARTED` recovery path), and in-memory counters would reset to zero on every restart, whereas query-derived metrics remain accurate. Retention metadata records successful sweeps even when they delete zero rows, distinguishing a completed no-op sweep from one that has never completed. Duration percentiles use a `(status, completedAt)` index to serve the windowed row search (no full-table scan); the bounded windowed set is then sorted and the nearest-rank percentile taken.
...
| Cursor SDK | server-side `CODEXGW_CURSOR_API_KEY` (user or service-account key) | Cursor plan request pools (SDK tag) |
