...
現在は制限付き Codex task API を実装しています。将来は、コーディングには ChatGPT/Codex 認証の Codex App Server、画像・音声には server-side の OpenAI Platform API を使う、個人用 AI capability Gateway へ拡張する方針です。

Gateway token、認可、監査、job、artifact、利用制限は共通化しますが、Codex thread や sandbox、画像生成、音声処理、realtime session の実行プロトコルは能力ごとに分離します。未実装機能を含む設計原則、認証・課金境界、API 方針、段階的な実装順は [`ARCHITECTURE.md`](ARCHITECTURE.md) を参照してください。
