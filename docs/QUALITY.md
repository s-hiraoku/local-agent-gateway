...
- `/readyz` probes App Server, requires a dedicated ChatGPT login, and fails closed when the Codex CLI version is outside the supported range. Operator login, job routing, and the private App Server method surface are in [Codex App Server](CODEX_APP_SERVER.md).
...
- Record live Lima acceptance evidence with `scripts/lima/accept.sh` plus ChatGPT-authenticated coding and inference runs. The opt-in executor fail-closes `/readyz` unless the guest tool-isolation probe passes. The default LaunchAgent still runs Codex on the host.
