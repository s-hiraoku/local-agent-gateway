...
- The Codex CLI is not installed in this environment. `GET /readyz` always probes Codex, so it will not report ready. Coding turns (`/v2/conversations/:id/turns`, `/v2/coding/runs`) need Codex. `/v2/inference/runs` needs the configured inference backend: Codex by default, otherwise Claude Code, Grok Build, or the Cursor SDK — none of which are present in this snapshot. Endpoints that exercise auth + encrypted SQLite persistence work without those backends: `GET /healthz`, `GET /v2/repositories`, `GET /v2/capabilities`, `POST /v2/conversations`, `GET /v2/metrics`, and the Swagger UI at `/docs`. Use those for smoke-testing a running server. `pnpm smoke` runs fully in-process with a fake runner and needs no Codex CLI.
